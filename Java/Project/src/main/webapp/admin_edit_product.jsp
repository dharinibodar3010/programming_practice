...
<%@ page import="com.model.AdminModel, com.model.ProductModel, com.dao.Dao" %>
<%
    ...
    ProductModel p = null;
    if(idStr != null) {
        p = Dao.getProductById(Integer.parseInt(idStr));
    }
...
<style>
    ...
    .card-edit { border: none; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.05); }
    ...
    .sidebar a:hover { color: white; background: rgba(255,255,255,0.1); border-left: 4px solid #0d6efd; }
    .current-img { width: 100px; height: 100px; border-radius: 10px; object-fit: cover; margin-bottom: 10px; border: 1px solid #ddd; }
</style>
</head>
<body>
    ...
    <div class="row">
        ...
        <div class="col-md-10 p-5">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="fw-bold m-0"><i class="bi bi-pencil-square me-2"></i> Edit Product</h2>
                <a href="admin_view_products.jsp" class="btn btn-outline-secondary btn-sm rounded-pill"><i class="bi bi-arrow-left"></i> Back</a>
            </div>
                ...
                <form action="AdminUpdateProductServlet" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="p_name" class="form-control" value="<%= p.getP_name() %>" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Price ($)</label>
                            <input type="number" step="0.01" name="p_price" class="form-control" value="<%= p.getP_price() %>" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Category</label>
                            <select name="category" class="form-select" required>
                                <option value="Men" <%= "Men".equals(p.getCategory()) ? "selected" : "" %>>Men</option>
                                <option value="Women" <%= "Women".equals(p.getCategory()) ? "selected" : "" %>>Women</option>
                                <option value="Kids" <%= "Kids".equals(p.getCategory()) ? "selected" : "" %>>Kids</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Current Image</label><br>
                            <% if(p.getP_image() != null) { %>
                                <img src="data:image/jpeg;base64,<%= p.getP_image() %>" class="current-img">
                            <% } %>
                            <br>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="p_des" class="form-control" rows="4" required><%= p.getP_des() %></textarea>
                        </div>
                        <div class="col-12 text-end mt-4">
                            <button type="submit" class="btn btn-primary btn-submit px-5 rounded-pill">Update Product</button>
                        </div>
                    </div>
                </form>
            </div>
