...
public class imageSave3 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ss != null) {
			...
			// byte[] imageData =
			// java.util.Base64.getDecoder().decode(base64Image.split(",")[1]);
			...
			try {
				...
				PreparedStatement ps = con.prepareStatement(
						"insert into cart(p_name,p_price,p_des,p_image,email,id) values(?,?,?,?,?,?)");

				// InputStream io = new
				// ByteArrayInputStream(image.getBytes(StandardCharsets.UTF_8));
